import { dogetindustryListNode } from "@Repo/AuthRepo";
import { IQuery } from "@Type/QueryType";
import { QueryKey, UseQueryOptions, useQuery } from "@tanstack/react-query";
import { Querykeys } from "./queryname";

type IProps = IQuery<UseQueryOptions<any, Error, any, QueryKey>>;

function useIndustryListNode(props: IProps) {
  return useQuery({
    queryKey: [Querykeys.industryList],
    queryFn: dogetindustryListNode,
    ...props,
  });
}

export default useIndustryListNode;
