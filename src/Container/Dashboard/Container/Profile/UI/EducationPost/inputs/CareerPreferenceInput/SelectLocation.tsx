...
export const SelectLocation = ({ locationArray, setLocationArray }) => {
  const { cityDataArray } = useSelector(
  ...
  const handleOptionSelect = (option: string, id: number) => {
    if (!locationArray.some((city) => city.id === id)) {
      setLocationArray([...locationArray, { name: option, id }]);
    }
    setInputValue("");
