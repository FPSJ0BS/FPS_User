...
export const Salary = ({setSelectedSalary}) => {
  const { salaryDataArray } = useSelector(
  ...
  const handleOptionSelect = (option: string, id: number) => {
    setSelectedSalary({ name: option, id });
  ...
  const filteredShifts = salaryDataArray
    .filter((option) =>
      option.salary.toLowerCase().includes(inputValue.toLowerCase())
    );
        ...
        <ul className="postjobHandleScrollbar max-h-[300px] overflow-y-auto absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg pl-0">
          {filteredShifts.slice(0, 50).map((option, index: number) => (
            <li
              ...
              onClick={() => handleOptionSelect(option.salary, option.ID)}
            >
              {option.salary}
            </li>
          ))}
