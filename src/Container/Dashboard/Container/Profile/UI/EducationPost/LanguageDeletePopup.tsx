...
import { closeModalLanguageDeleteModal, toggleRefetchProfile } from "@/Redux/Dashboard/MyProfile/Education/EducationSlice";
import { getDeleteLanguageDetails } from "@/api/api";
import { Toast } from "@Utils/Toast";

function LanguageDeletePopup() {
  const dispatch = useDispatch();
  const [buttonLoad, setButtonLoad] = useState(false);
  const { editLanguageData } = useSelector(
  ...
  const onSubmitData = async () => {
    setButtonLoad(true);
    try {
      const res = await getDeleteLanguageDetails(
        editLanguageData.languageId
      );

      if (res?.data?.status) {
        dispatch(toggleRefetchProfile())
        await dispatch(closeModalLanguageDeleteModal());
        setButtonLoad(false);
...
export default LanguageDeletePopup;
