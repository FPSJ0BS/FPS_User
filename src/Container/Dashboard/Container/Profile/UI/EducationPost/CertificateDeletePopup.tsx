...
import { closeModalCertificateDeleteDataModal,  toggleRefetchProfile } from "@/Redux/Dashboard/MyProfile/Education/EducationSlice";
import { getDeleteCertificateDetails } from "@/api/api";
import { Toast } from "@Utils/Toast";

function CertificateDeletePopup() {
  const dispatch = useDispatch();
  const [buttonLoad, setButtonLoad] = useState(false);
  const { editCertificateData } = useSelector(
  ...
  const onSubmitData = async () => {
    setButtonLoad(true);
    try {
      const res = await getDeleteCertificateDetails(
  ...
  const popupCloseFunc = async () => {
    await dispatch(closeModalCertificateDeleteDataModal());
  };
...
export default CertificateDeletePopup;
