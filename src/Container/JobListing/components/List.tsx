...
const List = ({ data, setQuery, query, refetch, setSearchJob, searchJob }) => {
  ...
  console.log("datadata?.packTypedata?.packTypedata", data);
  ...
  const setJobFavourite = (
    ...
    Favourite({
      ...
      if (res?.status === "success") {
        ...
        console.log("fav success", res);
        refetch();
      } else {
        Toast("error", res?.message);
        console.log("fail success", res);
      }
    });
  };
  ...
  return (
    <div className=" flex flex-col w-full h-full pr-[20px]">
      {
        <div className=" grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4  gap-4 w-full ml-0 cursor-pointer">
